...
from fairseq import utils, my_utils
...
#   sentence_end = tf.cast(tf.equal(inputs, sentence_sep_id), tf.float32) # we are here 
...
def generate_masking(inputs, sentence_sep_id):
    ...
    sentence_end_mask_row = sentence_end_mask.unsqueeze(dim=-1)
    sentence_end_mask_col = sentence_end_mask.unsqueeze(dim=-2)
...
class MyMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MyMultiheadAttention._append_prev_key_padding_mask(
...
@with_incremental_state
class MyLocalMultiheadAttention(nn.Module):
    """Multi-headed attention.
