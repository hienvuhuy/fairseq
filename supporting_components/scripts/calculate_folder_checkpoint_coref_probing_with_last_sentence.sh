...
coref_cluster_path='/home/is/huyhien-v/Data/Bin/MT/en-ru/coref-mt-probing-bos/cluster_with_bpe/'
...
for dir in "${RESULT[@]}"; do
    ...
    --task translation_coref_probing --load-source-coref --coref-cluster-path $coref_cluster_path
    
    # echo 
    # exit
    grep ^H $generate_file | cut -f3- > "$generate_file"."$sub_prefix".sys
    grep ^H $generate_file | cut -f3- | awk '{split($0, a, "_eos"); print a[4]}' | awk '{$1=$1};1' > $generate_file.$sub_prefix.lastsent.sys
    grep ^T $generate_file | cut -f2- > "$generate_file"."$sub_prefix".ref
    grep ^T $generate_file | cut -f2- | awk '{split($0, a, "_eos"); print a[4]}' | awk '{$1=$1};1' > $generate_file.$sub_prefix.lastsent.ref
    
    output=$(fairseq-score --sys $generate_file"."$sub_prefix".sys" --ref $generate_file"."$sub_prefix".ref")
    output_lastsent=$(fairseq-score --sys $generate_file"."$sub_prefix".lastsent.sys" --ref $generate_file"."$sub_prefix".lastsent.ref")
    # echo "$output"
    final_result="${output##*$'\n'}"
    final_lastsent_result="${output_lastsent##*$'\n'}"
    echo "$sub_prefix -- $final_result" >> $results_file
    echo "$sub_prefix -- $final_lastsent_result" >> $results_lastsent_file
