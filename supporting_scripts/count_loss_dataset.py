...
def main(cfg: DictConfig, override_args=None):
    ...
    for subset in cfg.dataset.valid_subset.split(","):
        ...
        losses = sorted(losses, key=lambda element: (element[0]))
        if cfg.common_eval.results_path is not None:
            os.makedirs(cfg.common_eval.results_path, exist_ok=True)
            output_path = os.path.join(
                cfg.common_eval.results_path,
                "generate-loss-{}.txt".format(cfg.dataset.gen_subset),
            )
            output_to_file = open(output_path, 'w')
            for item in losses:
                # output_to_file.write(str(item[1])+'\t'+str(item[2])+'\t|||'+str(item[3])+'\n')
                output_to_file.write(str(item[1])+'\n')
...
def cli_main():
    ...
    parser = options.get_counting_loss_parser()
