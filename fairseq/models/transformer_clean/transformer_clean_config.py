...
@dataclass
class EncDecCleanBaseConfig(FairseqDataclass):
...
@dataclass
class DecoderCleanConfig(EncDecCleanBaseConfig):
...
@dataclass
class QuantNoiseCleanConfig(FairseqDataclass):
    pq: float = field(
        default=0.0,
...
@dataclass
class TransformerCleanConfig(FairseqDataclass):
    ...
    encoder: EncDecCleanBaseConfig = EncDecCleanBaseConfig()
    ...
    decoder: DecoderCleanConfig = DecoderCleanConfig()
    ...
    quant_noise: QuantNoiseCleanConfig = field(default=QuantNoiseCleanConfig())
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = DecoderCleanConfig(**args.decoder)
                    else:
                        config.decoder = cls._copy_keys(
                            args, DecoderCleanConfig, "decoder", seen
                        )
                elif fld.name == "encoder":
                    # same but for encoder
                    if hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = EncDecCleanBaseConfig(**args.encoder)
                    else:
                        config.encoder = cls._copy_keys(
                            args, EncDecCleanBaseConfig, "encoder", seen
                        )
                elif fld.name == "quant_noise":
                    # same but for quant_noise
                    if hasattr(args, "quant_noise"):
                        seen.add("quant_noise")
                        config.quant_noise = QuantNoiseCleanConfig(**args.quant_noise)
                    else:
                        config.quant_noise = cls._copy_keys(
                            args, QuantNoiseCleanConfig, "quant_noise", seen
                        )
