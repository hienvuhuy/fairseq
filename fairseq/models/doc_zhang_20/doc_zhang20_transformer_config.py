...
@dataclass
class DocZhang20EncDecBaseConfig(EncDecBaseConfig):
    pass


@dataclass
class DocZhang20DecoderConfig(DecoderConfig):
    pass


@dataclass
class DocZhang20QuantNoiseConfig(QuantNoiseConfig):
    pass


@dataclass
class DocZhang20TransformerConfig(TransformerConfig):
    ...
    encoder: DocZhang20EncDecBaseConfig = DocZhang20EncDecBaseConfig()
    ...
    decoder: DocZhang20DecoderConfig = DocZhang20DecoderConfig()
    ...
    quant_noise: DocZhang20QuantNoiseConfig = field(default=DocZhang20QuantNoiseConfig())
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = DocZhang20DecoderConfig(**args.decoder)
                    else:
                        config.decoder = cls._copy_keys(
                            args, DocZhang20DecoderConfig, "decoder", seen
                        )
                elif fld.name == "encoder":
                    # same but for encoder
                    if hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = DocZhang20EncDecBaseConfig(**args.encoder)
                    else:
                        config.encoder = cls._copy_keys(
                            args, DocZhang20EncDecBaseConfig, "encoder", seen
                        )
                elif fld.name == "quant_noise":
                    # same but for quant_noise
                    if hasattr(args, "quant_noise"):
                        seen.add("quant_noise")
                        config.quant_noise = DocZhang20QuantNoiseConfig(**args.quant_noise)
                    else:
                        config.quant_noise = cls._copy_keys(
                            args, DocZhang20QuantNoiseConfig, "quant_noise", seen
                        )
