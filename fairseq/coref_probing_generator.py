...
class CorefProbingSequenceGenerator(object):    
    def __init__(
        self,
        combine_method,
        tgt_dict,
    ...
    def generate(
        ...
        **kwargs
        ...
        encoder_outs = model.reorder_encoder_out(encoder_outs, new_order) # why do they do that in the initialized step?

        src_lengths = encoder_input['src_lengths']
        # initialize buffers
        scores = src_tokens.new(bsz * beam_size, max_len + 1).float().fill_(0) # Khoi tao score with beam_size va max len
        lm_prefix_scores = src_tokens.new(bsz * beam_size).float().fill_(0) # score cua LM

        scores_buf = scores.clone() # ??? ko hieu
        tokens = src_tokens.new(bsz * beam_size, max_len + 2).long().fill_(self.pad) # add padding into new tokens array. hmm???
