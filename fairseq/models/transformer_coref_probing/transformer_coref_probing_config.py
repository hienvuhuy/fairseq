...
@dataclass
class EncDecCorefProbingBaseConfig(FairseqDataclass):
...
@dataclass
class DecoderCorefProbingConfig(EncDecCorefProbingBaseConfig):
...
@dataclass
class QuantNoiseCorefProbingConfig(FairseqDataclass):
    pq: float = field(
        default=0.0,
...
@dataclass
class BertRepresentationCorefProbingConfig(EncDecCorefProbingBaseConfig):
    input_dim: int = II("model.bert_representations.embed_dim")
    output_dim: int = field(
        default=II("model.bert_representations.embed_dim"),
        metadata={
            "help": "bert-representations output dimension (extra linear layer if different from bert-representations embed dim)"
        },
    )

    def __post_init__(self):
        #  II doesn't work if we are just creating the object outside of hydra so fix that
        if self.input_dim == II("model.bert_representations.embed_dim"):
            self.input_dim = self.embed_dim
        if self.output_dim == II("model.bert_representations.embed_dim"):
            self.output_dim = self.embed_dim
...
@dataclass
class TransformerCorefProbingConfig(FairseqDataclass):
    ...
    encoder: EncDecCorefProbingBaseConfig = EncDecCorefProbingBaseConfig()
    ...
    decoder: DecoderCorefProbingConfig = DecoderCorefProbingConfig()
    # TODO should really be in the decoder config
    
    # for bert_representation
    bert_representations: BertRepresentationCorefProbingConfig = BertRepresentationCorefProbingConfig()
    ...
    quant_noise: QuantNoiseCorefProbingConfig = field(default=QuantNoiseCorefProbingConfig())
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = DecoderCorefProbingConfig(**args.decoder)
                    else:
                        config.decoder = cls._copy_keys(
                            args, DecoderCorefProbingConfig, "decoder", seen
                        )
                elif fld.name == "encoder":
                    # same but for encoder
                    if hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = EncDecCorefProbingBaseConfig(**args.encoder)
                    else:
                        config.encoder = cls._copy_keys(
                            args, EncDecCorefProbingBaseConfig, "encoder", seen
                        )
                elif fld.name == "quant_noise":
                    # same but for quant_noise
                    if hasattr(args, "quant_noise"):
                        seen.add("quant_noise")
                        config.quant_noise = QuantNoiseCorefProbingConfig(**args.quant_noise)
                    else:
                        config.quant_noise = cls._copy_keys(
                            args, QuantNoiseCorefProbingConfig, "quant_noise", seen
                        )
