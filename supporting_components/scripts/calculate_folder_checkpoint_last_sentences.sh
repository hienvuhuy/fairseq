...
for dir in "${RESULT[@]}"; do
    ...
    fairseq-generate $data_path --path $dir --task $task --batch-size 128 --beam $beam --remove-bpe=subword_nmt --lenpen 0.6 --results-path $temp_directory
    
    # grep ^H $generate_file | cut -f3- > "$generate_file"."$sub_prefix".sys
    # grep ^T $generate_file | cut -f2- > "$generate_file"."$sub_prefix".ref
    
    # echo "grep ^H $generate_file | cut -f3- | awk '{split(\$0, a, \" $delim \"); print a[4]}' | awk '{\$1=\$1};1' > $generate_file.$sub_prefix.sys"
    grep ^H $generate_file | cut -f3- | awk '{split($0, a, "_eos"); print a[4]}' | awk '{$1=$1};1' > $generate_file.$sub_prefix.sys
    # echo "grep ^T $generate_file | cut -f2- | awk '{split(\$0, a, \" $delim \"); print a[4]}' | awk '{\$1=\$1};1' > $generate_file.$sub_prefix.ref"
